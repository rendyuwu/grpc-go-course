...
type CalculatorClient interface {
	...
	Averages(ctx context.Context, opts ...grpc.CallOption) (Calculator_AveragesClient, error)
...
func (c *calculatorClient) Averages(ctx context.Context, opts ...grpc.CallOption) (Calculator_AveragesClient, error) {
	stream, err := c.cc.NewStream(ctx, &Calculator_ServiceDesc.Streams[1], "/calculator.Calculator/Averages", opts...)
	if err != nil {
		return nil, err
	}
	x := &calculatorAveragesClient{stream}
	return x, nil
}

type Calculator_AveragesClient interface {
	Send(*AverageRequest) error
	CloseAndRecv() (*AverageResponse, error)
	grpc.ClientStream
}

type calculatorAveragesClient struct {
	grpc.ClientStream
}

func (x *calculatorAveragesClient) Send(m *AverageRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *calculatorAveragesClient) CloseAndRecv() (*AverageResponse, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
...
type CalculatorServer interface {
	...
	Averages(Calculator_AveragesServer) error
...
func (UnimplementedCalculatorServer) Averages(Calculator_AveragesServer) error {
	return status.Errorf(codes.Unimplemented, "method Averages not implemented")
}
...
func _Calculator_Averages_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CalculatorServer).Averages(&calculatorAveragesServer{stream})
}

type Calculator_AveragesServer interface {
	SendAndClose(*AverageResponse) error
	Recv() (*AverageRequest, error)
	grpc.ServerStream
}

type calculatorAveragesServer struct {
	grpc.ServerStream
}

func (x *calculatorAveragesServer) SendAndClose(m *AverageResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *calculatorAveragesServer) Recv() (*AverageRequest, error) {
...
var Calculator_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "Averages",
			Handler:       _Calculator_Averages_Handler,
			ClientStreams: true,
		},
		{
			StreamName:    "Max",
